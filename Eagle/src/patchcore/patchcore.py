...
class PatchCore(torch.nn.Module):
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False):
        ...
        # add attention layer3
        # features = self.attention_step(features)
        # ################# vit need fit the shape ################
        # for i in range(len(features)):
        #     B, N, D = features[i].shape
        #     features_vit = features[i]
        #     vit_feature = features_vit[:, 1:, :]
        #     vit_feature = vit_feature.permute(0,1,2)
        #     vit_feature= vit_feature.reshape(B,D,14,14)
        #     features[i] = vit_feature
        ################################
        features = [
    ...
    def _fill_memory_bank(self, input_data):
        ...
        # 保存特征到 .npy 文件
        # np.save("features_before_sampling.npy", features)
        # keep the same dimention shape between up and down lines
        # features = self.attention_step(features)
